...
fn main() {
    ...
    } else if env::var("TARGET").map(|v| v.contains("emscripten")).unwrap_or(false) {
        build_emscripten();
    } else {
        get_libsodium();
    }
...
fn build_emscripten() {
    ...
    build.include("libsodium\\src\\libsodium\\include\\sodium");
    build.define("SODIUM_STATIC", "1");

    let sources = vec!("crypto_aead\\chacha20poly1305\\sodium\\aead_chacha20poly1305.c",
    ...
    for source in sources.iter() {
        build.file(format!("libsodium\\src\\libsodium\\{}", source));
    }

    build.compile("sodium");
}
